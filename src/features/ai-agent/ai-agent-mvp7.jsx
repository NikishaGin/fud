...
async function getStore() {
    ...
    const mod = await import('../../app/store.js');
    storeRef = mod.store;
    return storeRef;
}


const WS_URL = 'ws://169.254.102.220:3003';
...
export function AiDialogView({ onClose }) {
    ...
    const handleCloseDialog = (uid) => {
        sendWs({ type: 'delete-dialog', uid });
    };

    const createNewDialog = () => {
        getStore().then(s => {
            const userId = s.getState().user.userId;
            const ws = wsRef.current;
            ws.send(JSON.stringify({
                type: 'create-dialog',
                userId,
                    ...
                    >
                        ...
                        <Box
                            sx={{
                                ...
                                flexGrow: 1,       // ← важно, контейнер занимает всё пространство
                                minWidth: 0,       // ← позволяет корректный ellipsis
                            }}
                        >
                        {dialogs.map((d) => {
                            ...
                            return (
                                <Box
                                    ...
                                    onClick={() => {
                                        setActiveUid(d.uid);
                                        setProcessingByDialog(prev => ({ ...prev, [d.uid]: false }));
                                        setActiveStatus(null);
                                        sendWs({ type: 'get-dialog', uid: d.uid });
                                        sendWs({ type: 'get-status', uid: d.uid }); // если у тебя нет такого — сделаем fallback ниже
                                    }}
                                >
                                    <Typography
                                        variant="caption"
