...
const MOCK_APPROVAL_L2 = {
    // 2-я линия:
    // URZ/UOPB — внутри количество ОТПРАВЛЕННЫХ (sentTotal) функций: approved / pending / rejected
    // MIUDOL — внутри ОБЩЕЕ количество функций MIUDOL: sent / notSent
    URZ:    { sentTotal: 160, parts: { approved: 120, pending: 30, rejected: 10 } },
    UOPB:   { sentTotal: 150, parts: { approved: 100, pending: 35, rejected: 15 } },
    MIUDOL: { total: 230, parts: { sent: 90, notSent: 140 } },
};

/* ================= helpers ================= */
function makeSlicesFromCounts(parts, paletteMap) {
    const keys = Object.keys(parts);
    const total = keys.reduce((a, k) => a + (Number(parts[k]) || 0), 0);
    const slices = keys.map((k) => {
        const count = Number(parts[k]) || 0;
        const percent = total > 0 ? (count / total) * 100 : 0;
        return { id: k, label: k, count, percent, color: paletteMap[k] };
    });
    return { total, slices };
}
...
function NineDonutsPanel() {
    ...
    const approvalL1 = ['URZ', 'UOPB', 'MIUDOL'].map((code) => {
        ...
        return {
            ...
            slices: [
                { label: 'На рассмотрении', ...functionMatchingFirstLine?.ACCEPT, color: PAL.info },
                { label: 'Согласовано', ...functionMatchingFirstLine?.APPROVED, color: PAL.ok },
                { label: 'Отклонено', ...functionMatchingFirstLine?.REJECTED, color: PAL.bad },
        ...
        return {
            ...
            slices: code !== 'MIUDOL'
                ? [
                    ...
                    { label: 'Отклонено', ...functionMatchingSecondLine?.REJECTED, color: PAL.bad },
                ...
                : [
                    ...
                    { label: 'Отклонено (сотрудниками ЦА)', ...functionMatchingSecondLine?.REJECTED, color: PAL.bad },
